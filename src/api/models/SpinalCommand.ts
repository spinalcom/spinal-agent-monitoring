import { Choice, Lst, Model, Ptr, spinalCore } from "spinal-core-connectorjs";
...
class SpinalCommand extends Model {
	constructor(type?: (typeof SPINAL_COMMAND_TYPE)[keyof typeof SPINAL_COMMAND_TYPE], processesIds?: string | number | (string | number)[], vmNode?: SpinalContext) {
		...
		this.add_attr({
			...
			createdAt: Date.now(),
			executedAt: null,
			executionResult: new Lst([]),
			vmNode,
		});
	}

	async execute(pm2ServiceInstance?: Pm2Service): Promise<ActionResponse[]> {
		try {
			if (!pm2ServiceInstance) pm2ServiceInstance = Pm2Service.getInstance();
	...
	async removeFromGraph(): Promise<boolean> {
